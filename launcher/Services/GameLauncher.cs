    ...
    public static class GameLauncher
    {
        ...
        public static void CleanupProxy(string gameDir)
        {
            ...
            TryDeleteMatching(
                ...
                Path.Combine(proxyDir, "winhttp.dll"));
            TryDeleteMatching(
                Path.Combine(gameDir, "doorstop_config.ini"),
                Path.Combine(proxyDir, "doorstop_config.ini"));
        }

        private static void TryDeleteMatching(string activePath, string storedPath)
        {
            try
            {
                if (!File.Exists(activePath) || !File.Exists(storedPath))
                    return;

                var activeInfo = new FileInfo(activePath);
                var storedInfo = new FileInfo(storedPath);
                if (activeInfo.Length != storedInfo.Length)
                    return;

                if (File.ReadAllBytes(activePath).AsSpan().SequenceEqual(File.ReadAllBytes(storedPath)))
                    File.Delete(activePath);
            }
            catch
            {
                // Cleanup is best-effort. Never remove an unknown bootstrap file.
            }
        }
    }
}
