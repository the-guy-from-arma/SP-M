...
namespace SeapowerMultiplayer
{
    ...
    public static class StateApplier
    {
        ...
        {
            return Globals._enemyTaskforce;
        }
        ...
        public static void Apply(PlayerOrderMessage msg)
        {
            ...
            try
            {
                switch (msg.Order)
                {
                    ...
                    case Messages.OrderType.Stop:
                        unit.setTelegraph(0);
                        unit.RemoveWaypoints();
                        break;

                    case Messages.OrderType.ClearOrders:
                        unit.RemoveWaypoints();
                        break;
    ...
    {
        public static void Apply(GameEventMessage msg, int peerId = -1)
        {
            Plugin.Log.LogInfo($"[Event] {msg.EventType} peer={peerId} src={msg.SourceEntityId} tgt={msg.TargetEntityId} param={msg.Param}");
            ...
            switch (msg.EventType)
            {
                ...
                case GameEventType.ControlRoleAssigned:
                    if (!Plugin.Instance.CfgIsHost.Value)
                ...
                case GameEventType.HardSyncRequest:
                    if (Plugin.Instance.CfgIsHost.Value)
                    {
                        Plugin.Log.LogWarning($"[HardSync] Peer {peerId} requested manual resync");
                        SessionManager.CaptureAndSend(peerId);
                    }
                    break;
                ...
                {
                    byte playerId = msg.SourceEntityId > 0 ? (byte)msg.SourceEntityId : (byte)1;
                    if (Plugin.Instance.CfgIsHost.Value && peerId >= 0)
                    {
                        var slot = FourPlayerLobby.FindByPeer(peerId);
                        if (slot == null) break;
                        playerId = slot.PlayerId;
                        NetworkManager.Instance.BroadcastToClientsExcept(peerId, new GameEventMessage
                        {
                    ...
                    UnitLockManager.OnRemoteSelected(playerId, (int)msg.Param);
                    break;
                }
                ...
                case GameEventType.UnitDeselected:
                {
                    ...
                    UnitLockManager.OnRemoteDeselected(playerId);
                    break;
                }
