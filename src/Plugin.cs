...
namespace SeapowerMultiplayer
{
    ...
    [BepInIncompatibility("com.seapowermultiplayer.plugin")]
    public class Plugin : BaseUnityPlugin
    {
        internal static ManualLogSource Log = null!;
        internal static Plugin Instance = null!;

        // --- Config entries (BepInEx/config/com.seapower.fourplayer.cfg) ---
        ...
        internal ConfigEntry<string> CfgPlayerName = null!;
        internal ConfigEntry<int> CfgPreferredSlot = null!;
        internal ConfigEntry<int> CfgPreferredTeam = null!;
        internal ConfigEntry<int> CfgPreferredRole = null!;
        internal ConfigEntry<string> CfgPublicLobbyName = null!;
        internal ConfigEntry<string> CfgServiceUrl = null!;
        ...
        private void Awake()
        {
            ...
            CfgTimeVote    = Config.Bind("Network", "TimeVote",     false,
                "Two-player compatibility mode only. Automatically disabled when more than two players are connected.");
            CfgPlayerName  = Config.Bind("Lobby", "PlayerName", System.Environment.UserName,
                "Name displayed in the four-player lobby");
            CfgPreferredSlot = Config.Bind("Lobby", "PreferredSlot", 0,
                "Preferred player slot: 0 = automatic, 2-4 = request that slot");
            CfgPreferredTeam = Config.Bind("Lobby", "PreferredTeam", 255,
                "Host: 0 = Blue, 1 = Red. Clients: 255 = automatic; slot 3 receives movement-only neutral navigation while exactly three players are connected.");
            CfgPreferredRole = Config.Bind("Lobby", "PreferredRole", 0,
                "Control role: 0 = Any, 1 = Surface, 2 = Submarine, 3 = Air, 4 = Land");
            CfgPublicLobbyName = Config.Bind("Lobby", "PublicLobbyName", "Open Fleet",
                "Operation name shown in the public launcher lobby browser");
            CfgServiceUrl = Config.Bind("Services", "DirectoryUrl",
                "https://spdash-production.up.railway.app",
                "Railway lobby directory and telemetry service URL");
            LobbyDirectoryClient.Configure(CfgServiceUrl.Value);
            ...
            // Multi-instance test harness: SP4P_* environment variables override the
            ...
            Log.LogInfo("Press Ctrl+F9 in-game to toggle the multiplayer UI overlay.");

            // Launcher/Steam arguments select a public host or a specific Steam lobby.
        ...
        /// <summary>
        /// SP4P_* environment variables override config values for this run.
        ...
        private void ApplyEnvOverrides()
        {
            ...
            string? role      = V("SP4P_ROLE");
            string? hostIp    = V("SP4P_HOSTIP");
            string? port      = V("SP4P_PORT");
            string? pvp       = V("SP4P_PVP");
            string? autoConn  = V("SP4P_AUTOCONNECT");
            string? transport = V("SP4P_TRANSPORT");
            string? playerName = V("SP4P_PLAYER_NAME");
            string? slot       = V("SP4P_SLOT");
            string? team       = V("SP4P_TEAM");
            string? roleName   = V("SP4P_CONTROL_ROLE");
            string? simLoss   = V("SP4P_NETSIM_LOSS");
            string? simLat    = V("SP4P_NETSIM_LATMS");

            if (role == null && hostIp == null && port == null && pvp == null
                && autoConn == null && transport == null && playerName == null
                && slot == null && team == null && roleName == null
                && simLoss == null && simLat == null)
                return;
            ...
            if (playerName != null) CfgPlayerName.Value = playerName;
            if (slot != null && int.TryParse(slot, out int requestedSlot)) CfgPreferredSlot.Value = requestedSlot;
            if (team != null && int.TryParse(team, out int requestedTeam)) CfgPreferredTeam.Value = requestedTeam;
            if (roleName != null && int.TryParse(roleName, out int requestedRole)) CfgPreferredRole.Value = requestedRole;
            ...
            Log.LogWarning($"[Config] SP4P_* env overrides active (role={(CfgIsHost.Value ? "host" : "client")}, " +
