...
namespace SeapowerMultiplayer.Messages
{
    public class DamageStateMessage : INetMessage
    {
        ...
        public float[] FloodingData = System.Array.Empty<float>();   // length = count * 2 * 4

        // Per SystemCompartment (count entries)
        // Each: FireSeverity, _fireGrowRate
        public float[] SystemData = System.Array.Empty<float>();     // length = count * 2

        // Damage control teams (count + 1 entries)
        public int[] DcTeams = System.Array.Empty<int>();

        // Per-system integrity (variable length across compartments)
        public byte   TotalSystemCount;
        public byte[] SystemCountsPerCompartment = System.Array.Empty<byte>();  // length = count
        public float[] SystemIntegrities = System.Array.Empty<float>();          // length = TotalSystemCount
        public byte[] SystemInoperables = System.Array.Empty<byte>();            // length = TotalSystemCount (0 or 1)
